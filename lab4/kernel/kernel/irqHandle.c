...
void syscallHandle(struct TrapFrame *tf) {
	...
	switch(tf->eax){
		...
		case SYS_seminit:sys_seminit(tf);break;
		case SYS_sempost:sys_sempost(tf);break;
		case SYS_semwait:sys_semwait(tf);break;
		case SYS_semdestroy:sys_semdestroy(tf);break;
